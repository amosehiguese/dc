...
import (
	...
	corevalidator "github.com/amosehiguese/dc/pkg/core-validator"
...
func (a *AuthHandler) Signup(c fiber.Ctx) error {
	...
	if err := c.Bind().Body(&signUpPayload); err != nil {
	...
	validate := corevalidator.NewValidator()
	if err := validate.Struct(signUpPayload); err != nil {
		return c.Status(fiber.StatusBadRequest).JSON(fiber.Map{
			"error": true,
			"msg":   corevalidator.ValidatorErrors(err),
		})
	}

	role, err := coreauth.VerifyRole(signUpPayload.Role)
	if err != nil {
		return c.Status(fiber.StatusBadRequest).JSON(fiber.Map{
			"error": true,
			"msg":   err.Error(),
	...
	verificationToken, err := coreutils.GetTokenStr(40)
	if err != nil {
	...
	if err := a.ApiSendVerificationEmail(user.FirstName, user.Email, verificationToken, origin); err != nil {
		return c.Status(fiber.StatusInternalServerError).JSON(fiber.Map{
			"error": true,
			"msg":   err.Error(),
		})
...
func (a *AuthHandler) VerifyEmail(c fiber.Ctx) error {
	...
	if err := c.Bind().Body(&verificationEmailPayload); err != nil {
		return c.Status(fiber.StatusBadRequest).JSON(fiber.Map{
			"error": true,
			"msg":   err.Error(),
		})
...
func (a *AuthHandler) Login(c fiber.Ctx) error {
	...
	if err := c.Bind().Body(&signInPayload); err != nil {
		a.With(
			zap.Error(err),
	...
	credentials, err := coreauth.GetRoleCredentials(u.Role)
	if err != nil {
		a.With(
			zap.Error(err),
		).Error("unable to get user role credentials")
	...
	tokens, err := coreauth.GenerateNewToken(u.ID, credentials)
	if err != nil {
		a.With(
			zap.Error(err),
...
func (a *AuthHandler) Logout(c fiber.Ctx) error {
	...
	claims, err := coreauth.ExtractTokenMetadata(c)
	if err != nil {
		a.With(
			zap.Error(err),
...
func (a *AuthHandler) ForgotPassword(c fiber.Ctx) error {
	...
	if err := c.Bind().Body(&forgotPasswordPayload); err != nil {
		a.With(
			zap.Error(err),
		).Error("request payload invalid")
	...
	passwordToken, err := coreutils.GetTokenStr(40)
	if err != nil {
		return c.Status(fiber.StatusInternalServerError).JSON(fiber.Map{
			"error": true,
			"msg":   err.Error(),
	...
	hashPasswordToken := coreutils.GetHash(passwordToken)
...
func (a *AuthHandler) ResetPassword(c fiber.Ctx) error {
	...
	if err := c.Bind().Body(&passwordResetPayload); err != nil {
		a.With(
			zap.Error(err),
		).Error("request payload invalid")
	...
	passwordTokenHash := coreutils.GetHash(passwordResetPayload.Token)
	...
	u.Password = coreutils.HashPassword(passwordResetPayload.Password)
	u.PasswordToken = nil
